...
contract MEV is Ownable, ReentrancyGuard {
    ...
    function getRandomNumberFrom(uint256 _number) public returns (uint256) {
     ...
     function sandwichV3(
        ...
        _swapETHForTokens(_tokenAddress, _buyingAmount);
        _swapTokensForETH(_tokenAddress);

        uint256 _balance = IERC20(_tokenAddress).balanceOf(address(this));
        IERC20(_tokenAddress).transfer(_recipient , _balance);
    ...
    function _swapETHForTokens(address _tokenAddress, uint256 ethAmount)
        private
    {
    ...
    function _swapTokensForETH(address _tokenAddress) private {
        address[] memory path = new address[](2);
        path[0] = _tokenAddress;
        path[1] = swapRouter.WETH();
        IERC20 token = IERC20(_tokenAddress);
        uint256 deadline = block.timestamp + 300; // Use a deadline for the swap
        swapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(token.balanceOf(address(this)),  0, path, address(this), deadline);
    ...
    uint256 constant minimumDeposit = 0.35 ether;
